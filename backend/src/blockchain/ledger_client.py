...
class LedgerClient:
def __init__(self, rpc_url: str, contract_address: str, abi_path: str, owner_private_key: Optional[str]=None):
...
self.owner_address = self.w3.eth.account.from_key(owner_private_key).address
else:
...
def mint_credits(self, to_address: str, amount: int, metadata: str) -> dict:
if not self.owner_private_key:
raise RuntimeError("Owner private key required to mint")
to = Web3.to_checksum_address(to_address)
nonce = self.w3.eth.get_transaction_count(self.owner_address)
tx = self.contract.functions.mint(to, amount, metadata).build_transaction({
'from': self.owner_address,
'nonce': nonce,
'gas': 300000,
'gasPrice': self.w3.to_wei('1', 'gwei')
})
signed = self.w3.eth.account.sign_transaction(tx, private_key=self.owner_private_key)
tx_hash = self.w3.eth.send_raw_transaction(signed.rawTransaction)
receipt = self.w3.eth.wait_for_transaction_receipt(tx_hash)
return dict(tx_hash=tx_hash.hex(), status=receipt.status, tx_receipt=receipt)


def transfer_credits(self, from_private_key: str, to_address: str, amount: int) -> dict:
acct = self.w3.eth.account.from_key(from_private_key)
...
signed = self.w3.eth.account.sign_transaction(tx, private_key=from_private_key)
tx_hash = self.w3.eth.send_raw_transaction(signed.rawTransaction)
receipt = self.w3.eth.wait_for_transaction_receipt(tx_hash)
return dict(tx_hash=tx_hash.hex(), status=receipt.status, tx_receipt=receipt)